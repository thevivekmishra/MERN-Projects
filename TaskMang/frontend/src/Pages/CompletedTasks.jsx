...
const CompletedTask = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/v1/task/completedtask", { headers });
  ...
  return (
    <>
    <h1 className='flex items-center justify-center bg-gray-700 p-3 text-xl rounded-lg md:mb-0 mb-[-30px]'>Completed Tasks</h1>
      {data.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-[80vh] text-center text-gray-500">
          ...
          <div className="text-xl font-semibold">No completed tasks available</div>
        </div>
      ) : (
